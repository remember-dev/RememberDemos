...
class _FoldersWithPriorityState extends State<FoldersWithPriority> {
  ...
  List<TaskRow> get unassignedTasks => lists[2];
  ...
  void onReorderUnassigned(int start, int end) {
    setState(() {
      if (end > start) end--;

      // Update the task priority if needed
      if (unassignedTasks[end].priority != unassignedTasks[start].priority) {
        unassignedTasks[start] = unassignedTasks[start]
            .copyWith(priority: unassignedTasks[end].priority);
      }

      unassignedTasks.insert(end, unassignedTasks.removeAt(start));
    });
  }
}
