...
class PlanningValues2 extends StatefulWidget {
  const PlanningValues2({super.key});

  @override
  State<StatefulWidget> createState() => _Planning();
}

class _Planning extends State<PlanningValues2> {
    ...
    return Scaffold(
      ...
      body: Column(children: [
        RememberPlanningTopBar(
          ...
          textSpans: [
            TextSpan(text: "This is some text. Lorem ipsum dolor sin amet...")
          ],
          currentStep: 1,
        ),
        Expanded(
        ...
        onPressedRight: () {
          if (selectedValues.isNotEmpty) {
            Navigator.push(
              context,
              MaterialPageRoute(
                builder: (_) => PlanningGoals2(valuesToShow: selectedValues),
              ),
            );
          } else {
            Navigator.pushAndRemoveUntil(
  ...
  void updateValue(PersonalValue personalValue, Category item) {
    if (personalValue.title.isNotEmpty) {
      ...
      setState(() {
        ...
        selectedValues.contains(personalValue);
  ...
  void onChanged(PersonalValue value) {
    bool contains = selectedValues.contains(value);
